...
export function renderProjectBridge({newTab = false} = {}) {
  const target = newTab ? ' target="_blank" rel="noopener noreferrer"' : '';
  return `<section class="library-project-bridge"><div><p class="eyebrow">La suite, c’est ton projet</p><h2>De tes lectures à un travail qui prend forme.</h2><p>Rassemble les consignes de ton encadrant, construis ton plan et prépare tes livrables modifiables dans ton espace Soutenance Pro.</p><p class="fine">1 projet gratuit · 3 générations par mois · Aucune carte bancaire</p></div><div class="library-project-actions"><a class="site-button" href="/#inscription"${target}>Commencer mon projet${newTab ? ' ↗' : ''}</a><a href="/tarifs"${target}>Comparer les offres${newTab ? ' ↗' : ''}</a></div></section>`;
}
