...
<header class="site-header"><a class="site-brand" href="/" aria-label="Soutenance Pro, accueil">${logo}</a><nav aria-label="Navigation principale"><a href="/bibliotheque">Bibliothèque</a><a href="/guides">Guides</a><a href="/outils">Outils</a><a href="/services">Mon projet</a><a href="/etablissements">Établissements</a><a href="/tarifs">Tarifs</a></nav><a class="site-button compact" href="/accompagnement">Accompagnement personnalisé</a></header>
${content}
...
export function renderPage(page, context) {
  ...
  const action=page.projectAction;
  const actionQuery=new URLSearchParams();
  if(['ispits','medecine'].includes(page.intent))actionQuery.set('parcours',page.intent);
  if(action)actionQuery.set('outil',action.module);
...
export function renderGuides(context) {
  ...
  const steps = [
    ...
    ['Structurer un mémoire', 'Relie la question, les matériaux accessibles et les conclusions possibles. Le parcours mémoire propose une grille de cadrage et un cas fictif commenté pour vérifier cette cohérence.', 'memoire', 'Construire mon mémoire'],
