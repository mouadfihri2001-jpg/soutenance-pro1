...
const HAL_FIELDS = 'halId_s,title_s,authFullName_s,producedDateY_i,docType_s,uri_s,fileMain_s,doiId_s';
const SUBJECTS = new Map(disciplines.map(subject => [subject.id, subject.halCode]));
const PARAMETERS = new Set(['q', 'discipline', 'type', 'year', 'page', 'pageSize']);
...
export function parseHalSearchQuery(query = {}) {
  ...
  if (type !== 'all' && !Object.hasOwn(documentTypes, type)) throw invalid('Type de document invalide.');
  if (year !== 'all' && (!/^\d{4}$/.test(year) || Number(year) < HAL_LIMITS.minYear || Number(year) > HAL_LIMITS.maxYear)) throw invalid('Choisis une année entre 2016 et 2026.');
  ...
  return { q, discipline, type, year, page: Number(page), pageSize: HAL_LIMITS.pageSize };
}
...
export function buildHalSearchUrl(search) {
  ...
  const filters = [
    'language_s:fr',
    'submitType_s:file',
    `docType_s:${search.type === 'all' ? '(ART OR THESE OR MEM OR REPORT OR HDR)' : search.type}`,
    `producedDateY_i:${search.year === 'all' ? '[2016 TO 2026]' : search.year}`
  ];
  ...
  url.searchParams.set('sort', 'producedDateY_i desc,docid asc');
  return url;
}
...
export function normalizeHalRecord(raw) {
  ...
  if (typeof id !== 'string' || id.length > 100 || !/^[a-z][a-z0-9-]+-\d+$/.test(id) || !title || title.length > 1200 || !authors.length || !sourceUrl || !fileUrl || !Object.hasOwn(documentTypes, raw.docType_s) || !Number.isInteger(year) || year < HAL_LIMITS.minYear || year > HAL_LIMITS.maxYear) return null;
  const doi = typeof raw.doiId_s === 'string' && raw.doiId_s.length <= 256 && /^10\.\d{4,9}\/[^\s<>"\\]+$/.test(raw.doiId_s) ? raw.doiId_s : '';
  return { id, title, authors, year, type: raw.docType_s, sourceUrl, fileUrl, doi };
}
...
export async function searchHal(query, fetchImpl = fetch, clock = () => new Date()) {
  ...
  try {
    ...
    return {
      records, total, page: search.page, pageSize: HAL_LIMITS.pageSize,
