...
export async function createWord(document,project) {
  ...
  const p=(text,options={})=>new Paragraph({bidirectional:rtl,spacing:{after:160,line:360},children:[new TextRun({text:plain(text),rightToLeft:rtl})],...options});
  ...
  for(const token of lexer(document.content)){
    if(token.type==='space')continue;
    if(token.type==='heading')children.push(p(token.text,{heading:HeadingLevel[`HEADING_${Math.min(token.depth,3)}`]}));
    else if(token.type==='list')token.items.forEach((item,index)=>children.push(p(`${token.ordered?`${index+1}.`:'•'} ${item.text}`)));
...
export async function createPowerPoint(document,project) {
  ...
  for(const [index,section] of presentationSections(document.content).entries()){
