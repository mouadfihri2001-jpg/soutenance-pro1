...
export function createChatHandler(auth = authenticate, send = (...args) => fetch(...args)) {
  return async function handler(req, res) {
    ...
    try {
      ...
      const { data: project, error: projectError } = await session.db.from('projects').select('*').eq('id', body.projectId).single();
      if (projectError || !project) throw new HttpError(404, 'Projet introuvable.');
      let planQuery = session.db.from('documents').select('content').eq('project_id', body.projectId).eq('module', 'plan');
      ...
      const { data: job, error: quotaError } = await admin.rpc('reserve_ai_job', { p_user_id: session.user.id, p_project_id: project.id, p_module: body.module });
      ...
      const { data: document, error: saveError } = await session.db.from('documents').insert({
      ...
      completed = true;
      await admin.from('ai_jobs').update({ status: 'completed', input_tokens: data.usage?.input_tokens || 0, output_tokens: data.usage?.output_tokens || 0 }).eq('id', jobId).then(() => {}, () => {});
      return res.status(200).json({ document, remaining: job.remaining, truncated: data.stop_reason === 'max_tokens' });
    } catch (error) {
      if (jobId && admin && !completed) await admin.from('ai_jobs').update({ status: 'failed' }).eq('id', jobId).then(() => {}, () => {});
