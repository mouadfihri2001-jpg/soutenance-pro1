...
function sessionMock({quota=true,saveError=false}={}){
 ...
 const db={from(table){
  const q={select(){return q;},eq(){return q;},order(){return q;},limit(){return Promise.resolve({data:[],error:null});},
   insert(value){writes.push(value);return q;},single(){return Promise.resolve(table==='projects'?{data:{id:projectId,title:'Mémoire',profile:{},sources:[]}}:saveError?{error:new Error('write failed')}:{data:{id:'doc',...writes.at(-1)}});}};
  return q;
 }};
...
test('the actual PostgreSQL schema isolates owners and enforces plan and quota limits',async()=>{
 ...
 try{
  ...
  await db.query('insert into projects(id,user_id,title) values ($1,$2,$3)',[projectId,owner,'Projet propriétaire']);
  ...
  assert.equal((await db.query("update projects set title='Intrusion' returning id")).rows.length,0);
  await assert.rejects(db.query("update accounts set plan='max'"),/permission denied/);
  await assert.rejects(db.query('select reserve_ai_job($1,$2,$3)',[other,projectId,'plan']),/permission denied/);
  await assert.rejects(db.query("insert into documents(user_id,project_id,module,title,content) values ($1,$2,'plan','Titre','Contenu')",[other,projectId]),/foreign key/);
  await db.exec(`select set_config('request.jwt.claim.sub','${owner}',false);`);
  assert.equal((await db.query('select * from projects')).rows.length,1);
  const documentId=(await db.query("insert into documents(user_id,project_id,module,title,content) values ($1,$2,'plan','Plan','Version initiale') returning id",[owner,projectId])).rows[0].id;
  await db.query("update projects set profile=jsonb_build_object('planValidated',true,'validatedPlanId',$1::text) where id=$2",[documentId,projectId]);
  await db.query("update documents set content='Plan modifié' where id=$1",[documentId]);
  assert.equal((await db.query('select profile from projects where id=$1',[projectId])).rows[0].profile.planValidated,false);
  await assert.rejects(db.query("insert into projects(user_id,title) values ($1,'Deuxième projet')",[owner]),/Limite/);
  await db.exec('reset role;');
  for(let i=0;i<3;i++)assert.equal((await db.query("select reserve_ai_job($1,$2,'plan') as job",[owner,projectId])).rows[0].job.allowed,true);
  assert.equal((await db.query("select reserve_ai_job($1,$2,'plan') as job",[owner,projectId])).rows[0].job.reason,'rate');
  await db.exec("update ai_jobs set created_at=now()-interval '2 minutes';");
  assert.equal((await db.query("select reserve_ai_job($1,$2,'plan') as job",[owner,projectId])).rows[0].job.reason,'quota');
  await db.exec("update ai_jobs set status='failed' where id=(select id from ai_jobs limit 1);");
  assert.equal((await db.query("select reserve_ai_job($1,$2,'plan') as job",[owner,projectId])).rows[0].job.allowed,true);
  await db.exec(`update accounts set plan='max',subscription_expires_at=now()-interval '1 day' where user_id='${owner}';`);
  await assert.rejects(db.query("insert into projects(user_id,title) values ($1,'Abonnement expiré')",[owner]),/Limite/);
  await db.exec(`set role authenticated; select set_config('request.jwt.claim.sub','${owner}',false);`);
  await db.query('delete from projects where id=$1',[projectId]);
  assert.equal((await db.query('select * from ai_jobs')).rows.length,4,'deleting a project must not reset the paid API quota');
  await db.exec('reset role;');
  await db.exec('set role anon;');await assert.rejects(db.query('select * from projects'),/permission denied/);
